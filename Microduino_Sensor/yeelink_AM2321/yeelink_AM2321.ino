...
char apiKey[] PROGMEM = "U-ApiKey: XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX";//replace to your APIKEY
...
void get_send_string(char *p, uint8_t type)
{
  ...
  case TEMP_TYPE:
    ...
    memset(temp, 0, 10);
    sprintf(p,"{\"value\":%s}", ftoa(temp, temperature, 1));
    break;

  case HUMI_TYPE:
    memset(temp, 0, 10);
    sprintf(p,"{\"value\":%s}", ftoa(temp, humidity, 1));
    break;
  }
