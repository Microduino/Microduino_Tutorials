/*
  Microgame.cpp - Arduino Microgame board library
...
// non-multiplexer Microgame pins:
// External outputs: D3, D11
// Buzzer: A8

_Microgame::_Microgame() {
  for (byte p=0; p<4; p++) {
    pinMode(MUX_ADDR_PINS[p], OUTPUT);
  }
}

unsigned int _Microgame::readChannel(byte channel) {
...
boolean _Microgame::joyLowHalf(byte joyCh) {
  return (readChannel(joyCh) < 512 - JOYSTICK_DEAD_ZONE)
    ? LOW : HIGH;
}

boolean _Microgame::joyHighHalf(byte joyCh) {
  return (readChannel(joyCh) > 512 + JOYSTICK_DEAD_ZONE)
    ? LOW : HIGH;
}

boolean _Microgame::readButton(byte ch) {
...
boolean _Microgame::readJoystickButton() {
...
boolean _Microgame::readJoystick1Button() {
...
void _Microgame::tone(unsigned int freq) {
  if (freq > 0)
...
void _Microgame::tone(unsigned int freq, unsigned long duration) {
  if (freq > 0)
