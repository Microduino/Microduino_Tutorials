...
#define AREA "+86"
#define NUM "13667928xxx"
#define SMS_CEN "13010720xxx"
...
void setup()
{
  ...
  Serial.println("if you input 't' in the terminal, the program will execute SendTextMessage()");
  Serial.println("if input 'd' in the terminal, it will execute DialVoiceCall()");
  Serial.println("=====WELCOME=====");
}
...
/*
  sprintf(buf, "AT+CSCA=\"%s%s\"",AREA,SMS_CEN); 
  Serial1.println(buf);
  for(int i=0; i<20; i++)
    buf[i]=NULL;
  delay(100);
...
*/
  ...
  sprintf(buf, "AT+CMGS=\"%s%s\"",AREA,NUM); 
  Serial1.println(buf);//send sms message, be careful need to add a country code before the cellphone number
  for(int i=0; i<20; i++)
    buf[i]=NULL;

  delay(100);
  Serial1.println("wiki.microduino.cc");//the content of the message
  delay(100);
